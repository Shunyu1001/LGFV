...
def write_full_control_outputs(specs: list[dict[str, object]]) -> None:
    ...
    with FULL_OUT_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write(
            ...
            "contemporary controls. Current city controls come from official "
            "statistical communiques, official budget or final-account reports, "
            "local-government bond disclosures, and secondary public-data "
            "compilations that should be replaced with official "
            "final-account tables where possible before this specification is "
            "treated as a final journal estimate. Land-finance proxy is "
            "government-fund revenue divided by general public budget revenue; "
            "one Huangshi entry uses city-direct government-fund revenue as a "
            "transparent first-pass proxy. "
            "Coefficients are linear probability "

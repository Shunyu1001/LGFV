...
def write_model_outputs(specs: list[dict[str, object]]) -> None:
    ...
    with OUT_MODEL_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\caption{Working-reference association between historical capacity and institutional change}\n")
        ...
        handle.write(
            ...
            "the CBDB-GADM historical-capacity measure. "
            f"{INDEPENDENT_CONFIRMATION_NOTICE} Coefficients are linear "
            "probability models with HC1 robust standard errors in parentheses. "
            "These estimates are working-reference descriptive associations rather than "
...
def write_capacity_bin_figure(rows: list[dict[str, str]]) -> None:
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel("Share of matched working-reference cases")

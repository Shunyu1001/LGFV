...
TIER_USE = {
    "human_validated": "Used as final validated labels",
...
def write_latex(rows: list[dict]) -> None:
    ...
    with OUT_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\caption{Validated cases and historical elite density}\\label{tab:pilot-historical-capacity}\\\\\n")
        handle.write("\\toprule\n")
...
def write_tier_latex(rows: list[dict]) -> None:
    ...
    with OUT_TIER_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\midrule\n")
        tier_rows = [
            ("Author-reviewed reference file", human_count, "Documentary outcome reference"),
            (
...
def write_distribution_figure(rows: list[dict]) -> None:
    ...
    with OUT_DISTRIBUTION_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\centering\n")
        handle.write("\\caption{Exit-type distribution in the validated evidence}\n")
...
def write_figure(rows: list[dict]) -> None:
    ...
    ax.set_ylabel("")
    ax.set_title("Historical elite density in validated cases")

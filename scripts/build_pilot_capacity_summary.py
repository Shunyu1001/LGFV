...
TIER_USE = {
    "human_validated": "Used as provisional working-reference labels",
...
def write_latex(rows: list[dict]) -> None:
    ...
    with OUT_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\caption{Working-reference cases and historical elite density}\\label{tab:pilot-historical-capacity}\\\\\n")
        handle.write("\\toprule\n")
        ...
        rows.append(
            {
                **row,
                ...
                "fiscal_absorption": mechanism.get(
                    "fiscal_absorption",
                    "Evidence not separately mechanism-coded in the current table.",
                ),
...
def write_tier_latex(rows: list[dict]) -> None:
    ...
    screening_summary = {
        row["quantity"]: int(row["value"])
        for row in read_csv(LLM_SCREENING_SUMMARY)
    }
    usable_screening = screening_summary["usable_llm_screening_rows"]
    surrogate_rows = screening_summary["screening_status:llm_surrogate_exit_type"]
    ...
    with OUT_TIER_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\caption{Measurement files used in the empirical analysis}\n")
        ...
        tier_rows = [
            ...
            (
                ...
                matched_count,
                "Used for exploratory descriptive tables",
            ),
            ("Candidate disclosure pool", seed_count, "Used for LLM-assisted screening"),
            ("Usable screening rows", usable_screening, "Source-backed screening coverage"),
            ("One-sided surrogate disclosure labels", surrogate_rows, "Provisional nominal-exit screens"),
        ]
...
def write_capacity_bin_latex(rows: list[dict]) -> None:
    ...
    with OUT_BIN_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\begin{table}[htbp]\n")
        handle.write("\\centering\n")
        handle.write("\\caption{Historical capacity bin and working-reference exit type}\n")
...
def write_distribution_figure(rows: list[dict]) -> None:
    ...
    with OUT_DISTRIBUTION_TEX.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\centering\n")
        handle.write("\\caption{Exit-type distribution in the working-reference evidence}\n")
...
def write_figure(rows: list[dict]) -> None:
    ...
    ax.set_ylabel("")
    ax.set_title("Historical elite density in working-reference cases")

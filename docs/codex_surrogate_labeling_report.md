...
- 73 human gold-standard labels
- 180 pending candidate disclosures screened by Codex
- 66 Codex surrogate disclosure-level labels
...
out of frame, the screening file contains 66 surrogate `nominal_exit` labels.
...
The 66 surrogate labels correspond to 45 unique issuers because several
Shanghai Clearing rows are repeated bond disclosures for the same platform. Of
these 45 issuers, 31 exactly match issuers that already appear in the
...
expansion. The remaining 14 issuers form the current validation queue. The
...
1,170 downloaded PDFs and 1,058 extracted text files. Among the 180 pending
candidate disclosures, 95 now have usable extracted text, 68 have downloaded
PDF records without usable extracted text, and 17 have no collected document
packet. The next collection task is to review the 14 queued non-overlap issuers

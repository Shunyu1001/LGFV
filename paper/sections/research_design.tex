...
case enters the working-reference file only after source-packet review of the
original or near-original documents under the codebook. Codex conducted the
initial review on behalf of the project author, who subsequently reported
that all reference labels had been checked by a human without revision. This separation prevents source
availability or a single compliance sentence from becoming an outcome by
default.

The current working-reference file contains ninety-four cases. It was assembled to
...
The explanatory application matches eighty-four reference cases to a crosswalk
of CBDB examination records and GADM boundaries. Elite density is used as a
historical proxy, not as a direct measure of present fiscal administration.
...
corrected full-control design uses district and county platforms as the
reference category and has full column rank. Adjusted results remain diagnostic
because the outcome contains only twelve institutional-change events and the
historical-capacity estimate is sensitive to case deletion.

The design keeps three inferential objects separate. The ninety-four working
reference cases provide human-checked outcomes for measurement development. The
...
provide observed outcomes. A downstream estimator that combines confirmed
reference outcomes and LLM labels requires a probability validation sample with
known inclusion probabilities and independent human coding. The current working
labels, although now human-checked according to the author's report, and the
post-hoc overlaps do not meet the probability-validation requirement.

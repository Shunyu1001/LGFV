...
cases. The full label file now contains eighty-seven validated labels; this
...
ninety-four human-validated labels as gold-standard observations and applies
the codebook only to pending candidates. The expanded candidate file contains
361 rows. Of these, 346 are now usable LLM screening observations:
ninety-four gold-standard exit labels, 203 LLM surrogate exit-type labels,
forty-four source packets screened as having no direct formal exit or
compliance event, and five human-reviewed boundary packets. Fifteen rows
remain unusable because the source packet is still missing. The exit-type
outcome sample is narrower: it contains 297 rows, consisting of the
ninety-four gold-standard labels and the 203 LLM surrogate exit-type labels.

The surrogate exit-type labels are nominal-exit cases. They contain direct
no-government-financing, no-new-government-debt, or no-hidden-debt compliance
language together with continuing evidence of urban infrastructure, entrusted
construction, land development, fiscal support, or public-project functions.
They correspond to 158 unique issuers because several
...
or city-platform level. Sixty-one issuers match platforms that already
appear in the gold-standard label file under different case IDs. These overlaps
are treated as consistency checks rather than new observations. Ninety-seven
non-overlap issuers remain in the current human-validation queue. The
source-screened rows without a direct formal event are not negative outcomes.
They are retained as screening observations because they document that usable
source text was reviewed under the codebook. They do not receive substantive,
nominal, functional-transfer, or liquidation labels.

\input{tables/llm_screening_status}

Table \ref{tab:surrogate-empirical-flow} then converts the disclosure-level
screening file into the issuer-level object used in the empirical section.
This step matters because repeated bond disclosures often describe the same
platform. The paper therefore does not count 203 disclosure-level surrogate
labels as 203 independent city-platform outcomes. It first deduplicates them
to 158 issuer-level surrogate rows, then separates the 61 rows that overlap
with the gold-standard file from the 97 non-overlap rows that can enter the
validation-adjusted descriptive sample.

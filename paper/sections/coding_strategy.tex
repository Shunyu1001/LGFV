...
cases. The full label file now contains sixty-eight validated labels; this
...
sixty-eight human-validated labels as gold-standard observations and applies the codebook
only to pending candidates. The current pass screens 187 pending disclosures, produces
73 surrogate disclosure-level labels, and leaves 114 cases unresolved. The
...
public-project functions. They correspond to 52 unique issuers because several
...
overlaps are treated as consistency checks rather than new observations. Twenty-one
non-overlap issuers remain in the current human-validation queue. Two
province-level Zhejiang transportation disclosures are boundary-reviewed and
excluded from the city-platform gold-standard frame. The unresolved cases are not negative

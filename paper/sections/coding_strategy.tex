...
cases. The full label file now contains fifty-nine validated labels; this
...
fifty-nine human-validated labels as gold-standard observations and applies the codebook
only to pending candidates. After removing four boundary-reviewed packets, the current pass screens 184 pending cases, produces
36 medium-confidence surrogate labels, and leaves 148 cases unresolved. The
...
public-project functions. They correspond to 23 unique issuers because several
...
or city-platform level. All 23 issuers exactly match issuers that
already appear in the gold-standard label file under different case IDs. These
overlaps are treated as consistency checks rather than new observations. The
current surrogate pass therefore leaves no issuer-level validation queue from
medium-confidence non-overlap labels. The unresolved cases are not negative

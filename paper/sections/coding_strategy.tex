...
303 rows. Of these, 288 are now usable LLM screening observations:
eighty-seven gold-standard exit labels, 131 LLM surrogate exit-type labels,
sixty-six source packets screened as having no direct formal exit or
compliance event, and four human-reviewed boundary packets. Fifteen rows
remain unusable because the source packet is still missing. The exit-type
outcome sample is narrower: it contains 218 rows, consisting of the
eighty-seven gold-standard labels and the 131 LLM surrogate exit-type labels.
...
public-project functions. They correspond to 98 unique issuers because several
...
or city-platform level. Fifty-four issuers match platforms that already
appear in the gold-standard label file under different case IDs. These overlaps
are treated as consistency checks rather than new observations. Forty-four

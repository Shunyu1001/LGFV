...
eighty-seven human-validated labels as gold-standard observations and applies
the codebook only to pending candidates. The full candidate file contains 257
rows. Of these, 242 are now usable LLM screening observations: eighty-seven
gold-standard exit labels, eighty-four LLM surrogate exit-type labels, sixty-five
source packets screened as having no direct formal exit or compliance event,
and six human-reviewed boundary packets. Fifteen rows remain unusable because
the source packet is still missing. The exit-type outcome sample is narrower:
it contains 171 rows, consisting of the eighty-seven gold-standard labels and
the eighty-four LLM surrogate exit-type labels.
...
public-project functions. They correspond to 64 unique issuers because several
...
or city-platform level. Thirty-seven issuers match platforms that already
appear in the gold-standard label file under different case IDs. These overlaps
are treated as consistency checks rather than new observations. Twenty-seven

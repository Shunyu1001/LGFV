...
working-reference file contains 94 Codex source-packet-reviewed cases,
subsequently checked by a human without revision according to the author's
report, each linked to an event,
...
auditable. The reported human check does not document blind double coding or
establish population accuracy, and the current cases were not drawn from a
...
population precision, recall, or multiclass error. Conditional on the linked
reference outcomes applying to the corresponding issuer and event, the
finite-screen nominal share remains bounded only between 38.61 and 100 percent.
The source-verified 67-issuer collection frame and tested finite-frame
estimators make the next validation step reproducible, but the collection
sheets do not yet contain human outcomes. The proposed census therefore does
not supply a completed validation-adjusted estimate.
...
stronger empirical application requires completed validation-frame human
decisions, documented independent coding, broader representation of rare
institutional fates, and stable contemporary controls. Those additions extend the measure;

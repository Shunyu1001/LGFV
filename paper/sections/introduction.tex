...
The author has confirmed that all labels were subsequently checked by a human
without revision. The file records two substantive
...
It excludes six low-capacity nominal exits. After correcting the former rank
deficiency, the adjusted effects remain close to zero and sensitive to
individual case deletion. These diagnostics rule out a strong claim that
...
labels \citep{egami2023dsl}. I report conditional finite-screen identification
bounds, prepare a source-linked 67-issuer validation frame, and implement the
finite-frame correction with explicit data requirements. The new frame has no
exact issuer/event matches to the existing human-check report. Its missing
human outcomes remain a collection requirement rather than being replaced by
surrogate labels.
